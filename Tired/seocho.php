...
//위험한 사고지역
$query = "select * from accident_data where sigugun_code=15 and state_code=0";
...
//신호등데이터
$query1 = "select * from cross_walk where 구코드=15";
$result1 = mysqli_query($connect, $query1);
$chart_data1 = '';
//중간 사고지역
$query2 = "select * from accident_data where sigugun_code=15 and state_code=1";
//state_code = 등급 0/1/2 =>가장 위험한거는? 0 = 위험 1=중간 2=경고
...
//약한 사고지역
$query3 = "select * from accident_data where sigugun_code=15 and state_code=2";
//state_code = 등급 0/1/2 =>가장 위험한거는? 0 = 위험 1=중간 2=경고
...
{
    $chart_data2 .= "'".$row["point_name"]."':[".$row["Latitude"].", ".$row["longitude"].",".$row['Occurrence'].",".$row['Casualty'].",".$row['dead_count'].",".$row['slanderer'].",".$row['Thin_embroidery'].",".$row['Injured_count'].",".$row['state_code']."], ";
}
//약한 사고데이터
while($row = mysqli_fetch_array($result3))
{
    $chart_data3 .= "'".$row["point_name"]."':[".$row["Latitude"].", ".$row["longitude"].",".$row['Occurrence'].",".$row['Casualty'].",".$row['dead_count'].",".$row['slanderer'].",".$row['Thin_embroidery'].",".$row['Injured_count'].",".$row['state_code']."], ";
}
...
<div class="header">
    <h1 class="page-header">
        &nbsp; &nbsp; &nbsp; &nbsp; 서초구  &nbsp; &nbsp;<img src="/test/img/icon7.png" alt="My Image">
...
<script id="code">
    ...
    var map = new naver.maps.Map('map', {
        //    127.0037297 | 37.48248422
        center: new naver.maps.LatLng(37.48248422,   127.0037297 ),
        zoom: 10,
        mapTypeControl : true
